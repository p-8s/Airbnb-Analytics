---
title: "ST309 Group Project Report: Analytics of Airbnb"
authors: "15702, 18196"
date: "18 Feb 2021"
output: pdf_document
---
...
In this section, we aim to visualise the listings across London on a map, and see for Superhost variation between boroughs. We also explore price differences. 
```{r}
...
## Price Map
Here we explore the price differentials between boroughs. 
```{r}
...
## 3.3 Property Data
## Property Type
There are a multitude of property types. Here we have grouped similar ones so as to reduce the total number of types to just 4. 
```{r}
...
## 3.4 Amenities Data
The amenities data requires heavy transformation into our code, as documented in the comments below. 
```{r}
...
## 3.5 Popularity
We devise a popularity index where a listing is popular conditional on obtaining above median reviews per month and above median review score. 
```{r}
#check no. of reviews
...
## 4.2 Popularity in the GLM
Here we attempt to check for correlations between listings' popularity and various variables.
```{r}
...
This time, we attempt to see if a decision tree for popularity can be fitted with our variables at hand. 
```{r}
...
- unfortunately, bagging and rf suggests the decision tree cannot be further improved

```{r}
#use a more balanced dataset to fit decision tree
...
Here we attempt to analyse character variables. Listing text might be incredibly important, as the descriptions serve to advertise listing characteristics to potential guests. 
```{r}
data(stop_words)
