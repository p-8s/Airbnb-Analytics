...
#convert empty entry in host_verification to NA
...
#attempts below
host_vecSam <- host_vnNew[1:100,]

nonNArows = !(is.na(host_vnClean))
host_vecSam[,]
km.out <- kmeans(host_vnNew, 4, nstart = 30)
cluster_restults <- km.out$cluster
cluster_feature <- km.out$centers

```


```{r}
host_ivn <- as.factor(listings1$host_identity_verified)
summary(host_ivn)
```
