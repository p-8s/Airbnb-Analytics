...
#load the shapefile of london wards
london_wards <- readOGR(dsn = "London-wards/London_Ward.shp") #the whole london-wards folder must be downloaded because .shp (shapefile) is composed by different files in the folder.
#transform the coordinates to longitude and latitude [1]
proj4string(london_wards) <- CRS("+init=epsg:27700")
london_wards.wgs84 <- spTransform(london_wards, CRS("+init=epsg:4326"))
  
head(london_wards_f)
#plot map
map1 <- ggplot() + geom_polygon(data=london_wards.wgs84, mapping=aes(long, lat, group = group), fill="lightskyblue3") + geom_path(data=london_wards.wgs84, mapping=aes(long, lat, group = group), color="white", lwd=0.1)
...
[1]  https://towardsdatascience.com/plotting-a-map-of-london-crime-data-using-r-8dcefef1c397 the reference for transformation of coordinates to longitude and latitude
