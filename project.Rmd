...
##(11.1) map one: visualise listings on map of london boroughs
#load the shapefile of london boroughs
ldn_boroughs0 <- readOGR(dsn = "London-boroughs/LondonBoroughs.shp") #the whole london-boroughs folder must be downloaded because .shp (shapefile) is composed by different files in the folder.
#transform the coordinates to longitude and latitude
ldn_boroughs <- ldn_boroughs0
...
map2 <- map1 + geom_point(data=lis_location, mapping=aes(longitude, latitude), color="dimgrey", size=0.05) + ggtitle("Airbnb listings in London")
map3 <- map2 + theme(legend.position = "none")

##(11.2) map two: visualise percentage of superhosts on map of london boroughs
...
ldn_boroughs_f <- left_join(ldn_boroughs_f, ldn_boroughs.wgs84@data, by = "id")

map01 <- ggplot(data = ldn_boroughs_f, aes(long, lat, group=group, fill=perc_superhost.y)) + geom_polygon() + geom_path(colour="white", lwd=0.05) + coord_equal() + scale_fill_gradient2(low = "#BDBDBD",high = "#2196F3", name = "% of superhosts")

map02 <- map01 + theme(legend.position = "none")
...
[3] https://github.com/valeria-io/crime_london_map (11.2) map 2
...
##use shape names to represent t/f for superhost 
