...
moved here because i need to merge is_popular with amenities data
...
#top 20 amenities for popular listings
popT_clean_list <- a_clean_list[popT.index]
popT_item_freq <- getMethodFreq(popT_clean_list, raw_items)
popT_item_freq <- as.data.frame(popT_item_freq)
popT_item_freq <- rownames_to_column(popT_item_freq, "item")
popT_item_freq <- popT_item_freq[order(popT_item_freq$freq, decreasing = T),]
popT_top20 <- popT_item_freq[1:20,]

#top 20 amenities for non-popular listings
popF_clean_list <- a_clean_list[-popT.index]
popF_item_freq <- getMethodFreq(popF_clean_list, raw_items)
popF_item_freq <- as.data.frame(popF_item_freq)
popF_item_freq <- rownames_to_column(popF_item_freq, "item")
popF_item_freq <- popF_item_freq[order(popF_item_freq$freq, decreasing = T),]
popF_top20 <- popF_item_freq[1:20,]
```
